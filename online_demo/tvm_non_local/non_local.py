...
class NL3DWrappe_MV2(nn.Module):
    def __init__(self, block, n_segment):
        super(NL3DWrappe_MV2, self).__init__()
        self.block = block
        self.nl = NONLocalBlock3D(block.conv[7].num_features)
        self.n_segment = n_segment

    def forward(self, x):

        x = self.block(x)
        # print(self.block.conv[7])
        # exit()
...
class NL3DWrappe_MV2_shift(nn.Module):
    def __init__(self, block, n_segment):
        super(NL3DWrappe_MV2_shift, self).__init__()
        self.block = block
        self.nl = NONLocalBlock3D(block.conv[7].num_features)
        self.n_segment = n_segment
...
def make_non_local_MV2(net, n_segment):
    import torchvision

    network = 'mobilenetv2'

    # print("non_local", NL3DWrappe_MV2_shift(net[5], n_segment, shift_buffer[1]))
    ...
    if network == 'mobilenetv2':
        net = nn.Sequential(
                ...
                net[3],
                net[4],
                NL3DWrappe_MV2_shift(net[5], n_segment),
                net[6],
                NL3DWrappe_MV2(net[7], n_segment),
                net[8],
                NL3DWrappe_MV2_shift(net[9], n_segment),
                net[10],
                NL3DWrappe_MV2(net[11], n_segment),
                net[12],
                NL3DWrappe_MV2_shift(net[13], n_segment),
                net[14],
                net[15],
                net[16],
                net[17],
